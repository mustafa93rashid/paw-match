    ...
    verifyMutation.mutate(
      { verificationCode: values.verificationCode },
      {
        onSuccess: () => {
          ...
          // reflect that immediately and send the adopter to sign in again.
          auth.clearSession();
          navigate(paths.login, { replace: true, state: { justEmailChanged: true } });
        },
        onError: (error) => {
          setVerifyError(
            getApiErrorMessage(error, "Could not verify the code. Please try again."),
          );
          const status = getApiStatus(error);
          if (status === 400) {
            const data = (error as { response?: { data?: { remainingAttempts?: number } } })
              .response?.data;
    ...
    return (
      ...
      >
        <p className="text-sm text-slate-600">
          We'll send a 6-digit verification code to your new email address. Your account email
          won't change until you verify that code.
        </p>
