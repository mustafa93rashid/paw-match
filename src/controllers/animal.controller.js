...
addAnimalImages = async (req, res) => {
  ...
  try {
    ...
    const isPrimary =
      animal.images.length === 0 && uploadedImages.length === 0;

    uploadedImages.push(
      this.buildUploadedImage(uploaded, isPrimary)
    );
...
// has at least one image and becomes available.
  ...
  // • If primary image is deleted, sets first image as primary.
  ...
  // ==================================================
  // • Allows Shelter Employees and Super Admin only.
  // • Shelter Employees can set primary images only for
  //   animals belonging to their assigned shelter.
  // • Super Admin can set primary images for any animal.
  // • Receives the image ID as a route parameter.
  // • Sets the specified image as primary and removes
  //   primary status from all other images.
  // ==================================================
  ...
  // ==================================================
  // • Allows Shelter Employees and Super Admin only.
  // • Shelter Employees can delete all images only from
  //   animals belonging to their assigned shelter.
  // • Super Admin can delete all images from any animal.
  // • Removes all images from the database first.
  // • Then attempts to remove them from Cloudinary.
  // • Cloudinary failures are logged but do not affect
...
deleteAllAnimalImages = async (req, res) => {
  ...
  const imageIds = animal.images.map((image) => image.publicId);
