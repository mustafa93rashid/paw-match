...
const updateAdopterProfileValidation = [
  allowOnlyFields(ADOPTER_PROFILE_FIELDS),

  requireAtLeastOneField(ADOPTER_PROFILE_FIELDS),
  ...
  body("homeType")
    ...
    .withMessage(
      "Home type must be either apartment, house, or farm",
    ),

  body("hasKids")
    .optional()
    .isBoolean({ strict: true })
    .withMessage("hasKids must be true or false"),
  ...
  body("experienceLevel")
    ...
    .withMessage(
      "Experience level must be either beginner, intermediate, or expert",
    ),
  ...
  body("dailyActivityLevel")
    ...
    .trim()
    .isString()
    .withMessage("Daily activity level must be a string")
    .bail()
    .isIn(["low", "medium", "high"])
    .withMessage(
      "Daily activity level must be either low, medium, or high",
    ),

  body("isAllergic")
    .optional()
    .isBoolean({ strict: true })
    .withMessage("isAllergic must be true or false"),

  body("ownerType")
    .optional()
    .trim()
    .isString()
    .withMessage("Owner type must be a string")
    .bail()
    .isIn(["single", "family"])
    .withMessage("Owner type must be either single or family"),
...
const updateShelterEmployeeProfileValidation = [
  ...
  allowOnlyFields(SHELTER_EMPLOYEE_WORK_FIELDS),

  requireAtLeastOneField(SHELTER_EMPLOYEE_WORK_FIELDS),
  ...
  body("employeeNumber")
    ...
    .trim()
    .isString()
    .withMessage("Employee number must be a string")
    .bail()
  ...
  body("hireDate")
    ...
    .trim()
    .isISO8601({ strict: true })
    .withMessage("Hire date must use the YYYY-MM-DD format"),

  validate,
];

// ==================================================
// Update vet profile validation
// ==================================================

const updateVetProfileValidation = [
  allowOnlyFields(VET_PROFILE_FIELDS),

  requireAtLeastOneField(VET_PROFILE_FIELDS),
  ...
  body("availableDays.*")
    ...
    .withMessage(
      "Available days must contain valid weekday names only",
    ),
  ...
  body("consultationTypes.*")
    ...
    .withMessage(
      "Consultation type must be either vetConsultation or behaviorTraining",
    ),
